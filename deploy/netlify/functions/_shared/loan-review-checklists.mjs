...
export const DSCR_DOCS = [
  ...
  { slug: 'credit_report', label: 'Credit Report', section: 'guarantor',
    conditions: 'Middle score above 690? Any lates or past-due accounts? Report is < 90 days old? The name on the report is the guarantor\'s full legal name per the government ID (GUARANTOR LEGAL NAMES OF RECORD), including the middle name. State which guarantor(s) this report covers - every guarantor needs one.' },
  ...
  { slug: 'loan_application', label: 'Loan Application / Credit Authorization', section: 'loan',
    conditions: 'All information filled out and accurate; signatures present; credit auth signed by every guarantor. The borrowing entity name on this application matches the recorded Articles of Organization (compare to the ENTITY NAME OF RECORD provided) - a mismatch is a defect on THIS application, not on the Articles; if the Articles have not been reviewed yet, mark that condition unclear. ' +
...
export const RTL_DOCS = [
  ...
  { slug: 'bank_stmt_current', label: 'Current-Month Bank Statements', section: 'borrower',
    ...
    // rubric used to ask for "two most recent statements", so the AI failed every
    // current-month tray for containing only one.
    conditions: 'ONE statement: the most recent monthly statement only (last month\'s statement is filed in its own tray — do NOT expect two statements here and do NOT fail this tray because the prior month is absent). Dated no more than 45 days before the note date; ALL pages present (no summary statement). Shows bank name, account number, account-holder name, and full transaction history. Available liquidity must cover down payment + 20% of the total rehab budget + 6 months of interest (PITIA). Asset weighting: checking/savings/money-market/CD/trust 100%; publicly traded stocks/bonds 70% of net; retirement 50% of vested (100% if the guarantor is 63+); business funds = the guarantor\'s ownership %. Crypto, sweat equity, and unsecured credit lines are excluded. Account holder: the borrowing entity (ENTITY NAME OF RECORD) or ANY of the guarantors listed in ACCEPTABLE ACCOUNT HOLDERS - any one of those names is acceptable. The account must be 100% owned by those parties: an account holder who is a non-guarantor person or a different entity is NOT acceptable (do NOT count it at 50% - flag it not_met and name the extra holder). Must be a FULL bank-generated statement or a bank-generated Account Transaction History - NOT a screenshot, photo, or partial capture of an online banking page.' },
...
// Deploy 237.075 (Mike) -- the mortgagee clause the Evidence of Insurance must carry.
// SLA-funded RTL/GUC: Sir Lends A Lot LLC or King Arthur Fund 1 LLC (whichever
// fund the loan is booked to; both listed when unknown). DSCR sold to DIYA:
// RE Investments Loans LLC. Mirrored in loan-doc-review.js _expectedMortgagee.
export const MORTGAGEE_SLA  = 'Sir Lends A Lot LLC, ISAOA/ATIMA';
export const MORTGAGEE_KAF  = 'King Arthur Fund 1 LLC, ISAOA/ATIMA';
export const MORTGAGEE_DIYA = 'RE Investments Loans LLC, ISAOA/ATIMA';
export function expectedMortgagee(opts) {
  const o = opts || {};
  const inv = String(o.investorName || '') + ' ' + String(o.investor || '');
  if (/diya|re investments/i.test(inv)) return MORTGAGEE_DIYA;
  if (/king arthur|\bkaf\b/i.test(inv)) return MORTGAGEE_KAF;
  if (/sir lends|sla capital|\bsla\b/i.test(inv)) return MORTGAGEE_SLA;
  if (String(o.loanType || '').toLowerCase() === 'dscr') return MORTGAGEE_DIYA;
  return MORTGAGEE_SLA + ' or ' + MORTGAGEE_KAF;
}
