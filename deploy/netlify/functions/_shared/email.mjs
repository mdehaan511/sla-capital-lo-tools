...
// Deploy 237.072 (Mike) -- every required document on a loan's review is in:
// tell the processor + admins the file is ready for underwriting.
export async function notifyFullFile({ toEmail, address, borrower, loanUrl, required }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) { console.warn('notifyFullFile: RESEND_API_KEY not configured'); return false; }
  if (!toEmail) return false;
  const escH = (s) => String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const where = String(address || borrower || 'a loan').trim();
  const n = Number(required || 0);
  const subject = 'Full file ready for underwriting: ' + where;
  const textBody = [
    'Every required document is in for ' + where + (borrower && address ? ' (' + borrower + ')' : '') + (n ? ' — ' + n + ' of ' + n + '.' : '.'),
    '',
    'Open the loan: ' + loanUrl,
    '',
    'SLA Capital',
  ...
  const html =
    ...
    '<div style="max-width:620px;margin:0 auto;font-family:Georgia,serif">' +
      '<div style="background:#261A36;padding:24px">' +
        '<h1 style="color:#C8813A;margin:0;font-size:18px">&#128193; Full file ready for underwriting</h1>' +
      '</div>' +
      '<div style="padding:24px;color:#1A1520">' +
        '<p style="font-size:15px;line-height:1.6">Every required document is in for <strong>' + escH(where) + '</strong>' +
          (borrower && address ? ' (' + escH(borrower) + ')' : '') + (n ? ' — ' + n + ' of ' + n + '.' : '.') + '</p>' +
        '<p style="margin:0 0 24px"><a href="' + escH(loanUrl) + '" style="display:inline-block;background:#C8813A;color:#fff;text-decoration:none;font-weight:700;padding:11px 20px;border-radius:8px;font-size:14px">Open the loan</a></p>' +
        '<p style="font-size:12px;color:#7A7488">Or copy this link: ' + escH(loanUrl) + '</p>' +
        '<p style="font-size:12px;color:#7A7488;margin-top:24px">Sir Lends A Lot LLC dba SLA Capital.</p>' +
      '</div>' +
  ...
  try {
    const resp = await fetch('https://api.resend.com/emails', {
      ...
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + apiKey, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: 'SLA Capital <noreply@leads.slacapital.com>', to: [toEmail], subject, text: textBody, html }),
    });
    if (!resp.ok) {
      const t = await resp.text().catch(() => '');
      console.warn('notifyFullFile: Resend ' + resp.status, t.slice(0, 200));
      return false;
    }
    return true;
  } catch (e) {
    console.warn('notifyFullFile: fetch threw:', e && e.message);
    return false;
  }
}
