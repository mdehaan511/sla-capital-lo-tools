...
async function handle(req, context) {
  ...
  // 1) Build (owner → set of normalized loan addresses) map.
  //    A quote is "matched" if its address (full or street-only) lines
  //    up against any loan on any client for that same owner.
  const loansByOwner = {};
  try {
    const { blobs } = await clientsStore.list();
    for (const { key } of blobs) {
      // client keys are typically "<ownerKey>/<clientId>"
      const slash = key.indexOf('/');
      if (slash < 0) continue;
      const ownerKey = key.slice(0, slash);
      const client = await clientsStore.get(key, { type: 'json' });
      if (!client || !Array.isArray(client.loans)) continue;
      const set = loansByOwner[ownerKey] || (loansByOwner[ownerKey] = { full: new Set(), street: new Set() });
      for (const loan of client.loans) {
        if (!loan || !loan.address) continue;
        set.full.add(normAddr(loan.address));
        const sp = streetPart(loan.address);
        if (sp) set.street.add(sp);
      }
    }
  } catch (e) {
    return json(500, { error: 'Failed to load clients: ' + (e.message || 'unknown') });
  }
  ...
  try {
    ...
    for (const { key } of blobs) {
      ...
      const borrowerEmail = (fd.borrowerEmail || quote.borrowerEmail || '').trim();
      const borrowerName  = (fd.borrower || fd.borrowerName || quote.borrower || '').trim();
      const borrowerPhone = (fd.borrowerPhone || quote.borrowerPhone || '').trim();
