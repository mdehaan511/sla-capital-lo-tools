...
function render() {
  ...
  // Deploy 237.158 (Mike) -- "Company On Docs" is the entity on the note. It was
...
function ldDrawsLoad(force){
  ...
  var num = (window.SLA && typeof SLA.slaLoanNumber === 'function')
    ? SLA.slaLoanNumber(_loan)   // Deploy 237.158 — shared with the Closed Loans Draws tab
...
function _deriveSlaLoanIdClient(loan) {
  if (!loan) return '';
  // Deploy 237.158 — the same rule now lives in sla-api.js (SLA.deriveSlaLoanNumber)
...
function saveFundingPlan() {
  ...
  var fields = {
    ...
    // Deploy 237.158 (Mike) — Company On Docs = the entity on the note. Mirrored
