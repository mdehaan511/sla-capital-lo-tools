#!/usr/bin/env node
/**
 * scripts/prepaid-interest-fees-test.mjs — Deploy 237.255 / 237.262 (Mike)
 *
 ...
 *
 * 237.262: "change how that new Prepaid Interest field is calculated to do 30/360 instead
 * of 30/365" -- the daily rate is on a 360-day year now (the Loan Terms card's own figure).
 *
 ...
 *     card already uses, on a 360-day year -- the SAME daily figure as that card);
...
const names = ['_ldParseYmd', '_ldDrawsNum', '_ldRatePctOf', '_days30360ToNextFirst', '_isGucLoan', '_ldMoney2', '_ldPrepaidInterest30360', '_fmtMoney0', '_feeRow', '_feesReserveParts', 'escH'];
const stubs = `
  var window = {};
  function _ldRehabHoldback(l) { return (l && l.rehabBudget) || 0; }
`;
const fns = new Function(flat + '\n' + stubs + names.map(extract).join('\n') + '\nreturn { ' + names.join(', ') + ' };')();
const { _days30360ToNextFirst: days, _ldPrepaidInterest30360: prepaid, _feesReserveParts: parts } = fns;
...
console.log('\nThe daily rate is on a 360-day year');
{
  const jeremy = { loanAmt: '281200', rate: '7.250', toolType: 'dscr', fundingDate: '2026-09-23' };
  const p = prepaid(jeremy, '2026-09-23', '');
  check('$281,200 at 7.250%: $56.63 a day, 8 days, $453.04', [r2(p.daily), p.days, r2(p.prepaid), p.hasDate, p.dutch], [56.63, 8, 453.04, true, true]);
  check('...which IS the Loan Terms card\'s 30/360 daily ($56.63) -- the two cards agree now', r2(p.daily), r2(281200 * 0.0725 / 360));
  check('...and not the 237.255 figure on a 365-day year ($55.85)', r2(p.daily) !== r2(281200 * 0.0725 / 365), true);
  ...
  check('Non-Dutch: on the initial advance (loan less the holdback), 30 days', [q.base, r2(q.daily), q.days, r2(q.prepaid)], [117000, 34.13, 30, 1023.75]);
  check('the live structure select wins over the loan on file', prepaid(rtl, '2026-10-01', 'dutch').base, 206000);
  check('the card\'s own amount and rate win over the loan record (still less the holdback when Non-Dutch)', [prepaid(rtl, '2026-10-01', '', 250000, 12).base, r2(prepaid(rtl, '2026-10-01', '', 250000, 12).daily)], [161000, 53.67]);
  check('...and on the full amount when Dutch', prepaid(rtl, '2026-10-01', 'dutch', 250000, 12).base, 250000);
  check('a rate stored as a decimal (0.105) is 10.5%', r2(prepaid({ loanAmt: '100000', rate: '0.105' }, '2026-10-01', '').daily), 29.17);
...
console.log('\nThe Fees / Cash to Close card');
{
  ...
  check('the line is on the card, labelled with the days and the method', rowLbl(dscr.fees, 'Prepaid Interest'), 'Prepaid Interest (8 days · 30/360)');
  check('...with the amount', rowAmt(dscr.fees, 'Prepaid Interest'), '$453');
  // origination 2 pts = 5,624 + flat 995+700+500+200 = 2,395 → 8,019 + 453 = 8,472 ; cash to close = 70,300 + 8,472
  check('Total Fees includes it', rowAmt(dscr.fees, 'Total Fees'), '$8,472');
  check('Estimated Cash to Close includes it', rowAmt(dscr.fees, 'Estimated Cash to Close'), '$78,772');
  check('the note names the daily figure', /Prepaid interest is the daily interest \(30\/360\) from the Closing Date through the end of that month — \$56\.63 a day\./.test(dscr.fees), true);
  ...
  check('close on the 1st instead: 30 days', [rowLbl(moved.fees, 'Prepaid Interest'), rowAmt(moved.fees, 'Prepaid Interest'), rowAmt(moved.fees, 'Total Fees')], ['Prepaid Interest (30 days · 30/360)', '$1,699', '$9,718']);
  ...
  // dutch (default on RTL): 206000*.105/360*30 = 1802.50 ; fees 4120 + 2150 + 1803 = 8073 ; ctc = 23000 + 8073 = 31073
  check('RTL: on the full note when Dutch, 30 days', [rowLbl(r.fees, 'Prepaid Interest'), rowAmt(r.fees, 'Prepaid Interest'), rowAmt(r.fees, 'Total Fees'), rowAmt(r.fees, 'Estimated Cash to Close')], ['Prepaid Interest (30 days · 30/360)', '$1,803', '$8,073', '$31,073']);
  check('...and the Cash Reserve starts from that Cash to Close', rowAmt(r.reserve, 'Cash to Close'), '$31,073');
  ...
  check('the live Non-Dutch selection: on the initial advance ($1,024)', rowAmt(nd.fees, 'Prepaid Interest'), '$1,024');
